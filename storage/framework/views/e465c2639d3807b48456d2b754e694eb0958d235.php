...
$(document).ready(function(){
    ...
    $(document).on('click','.salvar_btn',function(e){
        ...
        var id = $('#add_equipamento_id').val();
        var grupoid = $('#add_grupo_id').val();
        var loading = $('#imgadd');
            loading.show();
        ...
        var orgaoid = selectorgao.value;
         ...
         $.ajaxSetup({
            ...
            $.ajax({
                type:'GET',
                dataType:'json',
                url:'/datacenteradmin/equipamento/carregasetores2/'+orgaoid+'/'+id,
                success:function(response){
                    if(response.status==200){
                        var equipamentosetor = response.equipamentosetor;
